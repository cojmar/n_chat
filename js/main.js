	...
	requirejs([
		...
		$(function() {
			...
			net.has_profanity = function(str, language) {
				...
				if (language === 'en') {
					...
					for (var r3 in blacklist_search_regex_tr) {
						if (str.match(blacklist_search_regex_tr[r3])) {
							return true;
						}
					}
				}
			...
			net.clean = function(str, sent_by_admin, disable_emoji) {
				...
				var subject_clean = net.remove_spam(net.remove_duplicates(net.remove_numbers(net.normalize(net.remove_zalgo(str.replace(/[-_*?!.,:;()'"`|\\/]/g, '').replace(/&lt;/g, '').replace(/&gt;/g, ''))))));
			...
			net.clean_nicknames = function(str, disable_emoji) {
				...
				var subject_clean = net.remove_spam(net.remove_duplicates(net.normalize(net.remove_zalgo(str.replace(/[-_*?!.,:;()'"`|\\/]/g, '').replace(/&lt;/g, '').replace(/&gt;/g, '')), normalize_types.slice(0, normalize_types.length - 1))));
				...
				if (!net.render_users_timeout || force) {
					net.render_users_timeout = setTimeout(function() {
						...
						for (var users in net.room_info.users) {
							...
							// noinspection JSUnfilteredForInLoop,JSUnresolvedVariable
							var country = net.room_info.users[users].data.country ? net.room_info.users[users].data.country + ' ' + country_codes[net.room_info.users[users].data.country] : '?';
